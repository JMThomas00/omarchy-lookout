#!/usr/bin/env python3
"""Loopback listener for the Google OAuth redirect.

Adapted from io.github.jeremylanger.omaspotify's scripts/oauth-callback.py
(MIT, Copyright (c) 2026 Omarchy Spotify contributors / OmaSpotify
contributors -- see ../THIRD_PARTY_LICENSES.md). That version only checked
the request path, since Spotify's redirect needs nothing else read here.
Google's Device Access redirect carries `code`/`state`/`error` as query
parameters on the same path -- this version is otherwise identical (same
byte budgets, same per-connection timeout, same signal handling) except
that it hands the whole accepted request line back to its caller verbatim,
including the query string, and leaves parsing/validation to the caller
(Lookout's OAuth.js, run in QML) rather than doing any of it here.

This reads a request line into a fixed budget and gives up the moment it is
exceeded, holds a deadline over the whole wait, ignores anything that is not
a GET for the expected path, and prints exactly one line: the request line
it accepted. The caller checks the OAuth state and sends back the response
to write, base64 on a single line, because the response itself has newlines
in it.
"""
...
def request_line_targets(line, expected_path):
    ...
    Only the path is checked here -- the query string (code/state/error) is
    read and validated by the caller, not this listener, matching the
    upstream design's split between a dumb bounded pipe and the interpreter
    ...
    return path == str(expected_path or "/oauth/callback")
...
def main(argv=None):
    ...
    parser.add_argument("--path", default="/oauth/callback")
